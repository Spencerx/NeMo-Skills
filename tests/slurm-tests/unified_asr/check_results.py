...
def load_references() -> dict[str, str]:
    container_path = Path("/nemo_run/code/tests/slurm-tests/unified_asr/asr_openai.test")
    local_path = Path(__file__).resolve().parent / "asr_openai.test"
...
def check_asr_results(workspace: str):
    ...
    for row in rows:
        ...
        if isinstance(debug_info, dict) and isinstance(debug_info.get("words"), list) and debug_info["words"]:
            found_debug_words += 1

    soft_assert(found_debug_words > 0, "No outputs contained debug_info.words from nemo_asr backend")
...
def main():
    ...
    check_asr_results(args.workspace)
    assert_all()
