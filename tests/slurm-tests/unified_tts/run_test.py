...
DEFAULT_CODEC = "nvidia/nemo-nano-codec-22khz-1.89kbps-21.5fps"
...
def run_unified_tts_test(
    ...
    input_file = "/nemo_run/code/tests/slurm-tests/unified_tts/tts_openai.test"
    output_dir = f"{workspace}/tts_outputs"
    mount_paths = f"{Path(workspace).parent}:{Path(workspace).parent}"
    server_args = f"--backend magpie_tts --codec_model {codec_model} --batch_size 6"

    if code_path:
        # Allow passing one or more colon-separated code roots.
        code_paths = [p for p in code_path.split(":") if p]
        mount_paths = ",".join([mount_paths] + [f"{p}:{p}" for p in code_paths])
...
def main():
    ...
    parser.add_argument("--model", default=DEFAULT_MODEL, help="Magpie model path/name")
    parser.add_argument("--codec_model", default=DEFAULT_CODEC, help="Codec model path/name")
    ...
    parser.add_argument(
        "--code_path",
        default=None,
        help="Optional colon-separated path(s) to prepend to PYTHONPATH in server container",
    )
    parser.add_argument(
    ...
    tts_expname = run_unified_tts_test(
